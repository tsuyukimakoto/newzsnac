...
test("runtime workers collect into SQLite, enqueue analysis, and persist LM results", async () => {
  ...
  try {
    ...
    assert.deepEqual(dashboard.ok && dashboard.data, { total: 1, unread: 1, saved: 0, readLater: 0, interested: 0, recommended: 0, pending: 0, failed: 0, analysisFailed: 0, readingMinutes: 1 });
...
test("dashboard operations return only persisted counts and sources", async () => {
  ...
  try {
    ...
    assert.deepEqual(empty.ok && empty.data, { total: 0, unread: 0, saved: 0, readLater: 0, interested: 0, recommended: 0, pending: 0, failed: 0, analysisFailed: 0, readingMinutes: 0 });
