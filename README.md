...
- `j`、`k`を中心としたキーボード操作と、原文確認を後工程へ分ける「あとで読む」一覧
...
通常のIndexで`b`を押すと、選択中の記事を「あとで読む」へ追加して既読にし、次の記事へ進みます。「保存済み」は本文や記事を手元に残す分類で、「あとで読む」は公開元を後から確認するための一時的な一覧です。二つの状態は独立しています。

左側の「あとで読む」では、記事タイトルが公開元への通常のリンクになります。タイトルを⌘クリックすると、バックグラウンドタブで開くかどうかを含め、ブラウザの通常のリンク操作として扱われます。

各記事のチェックボックスは削除対象の選択です。既定ではすべてチェック済みで、「選択を一括削除」を押すと、チェックした記事だけを「あとで読む」から削除します。既読、保存済み、「気になった」の状態は変更しません。

「あとで読む」では`j`で現在の記事を一覧から除外して次へ進みます。`k`で戻る場合、移動前の記事は一覧から除外しません。`o`はほかのIndexと同じく、現在の記事だけを新しいタブで開きます。

タブを前面と背景のどちらで開くかは、ブラウザの設定と修飾キー操作に従います。Newzsnacから複数タブを一括生成する機能はありません。
